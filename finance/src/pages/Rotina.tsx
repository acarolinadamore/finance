...
import { useRoutines, useCreateRoutine, useUpdateRoutine, useDeleteRoutine, useReorderRoutines, useToggleRoutineCompletion, useRoutineCompletions } from '@/hooks/useApiRoutines';
import { useHabits, useCreateHabit, useUpdateHabit, useDeleteHabit, useArchiveHabit, useToggleHabitCompletion, useHabitCompletions } from '@/hooks/useApiHabits';
...
import { Period, Frequency, PERIOD_LABELS, EMOTIONS, Routine, Habit, DailyMood } from '@/types/routine';
...
const SortableRoutineCard = (props: SortableRoutineCardProps) => {
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <RoutineCard {...props} dragHandleProps={{ ...attributes, ...listeners }} />
    </div>
  );
};
...
const Rotina = () => {
  ...
  const handleAddRoutine = async (routineData: RoutineFormData) => {
    if (editingRoutine) {
      ...
      await createRoutineMutation.mutateAsync({
        name: routineData.name,
        period: routineData.period,
        frequency: (routineData.frequency || 'daily') as 'daily' | 'weekly' | 'custom',
        specific_days: routineData.specificDays,
        times_per_week: routineData.timesPerWeek,
        icon: routineData.icon,
  ...
  const filteredHabits = showArchivedHabits ? domainHabits : domainHabits.filter((h) => h.isActive);
  ...
  const getRoutinesByPeriod = (period: Period) => {
    return domainRoutines.filter(r => r.period === period && r.isActive !== false);
  };
  ...
  const completedRoutines = domainRoutines.filter((r) => isRoutineCompleted(r.id)).length;
  const progressPercentage = totalRoutines > 0 ? Math.round((completedRoutines / totalRoutines) * 100) : 0;

  console.log('✅ [Progresso] Total:', totalRoutines, 'Completadas:', completedRoutines, 'Porcentagem:', progressPercentage);
                ...
                <div className="space-y-2">
                  {periodRoutines.map((routine) => (
                    <SortableRoutineCard
                      ...
                      onToggle={() => handleToggleRoutine(routine.id, !isRoutineCompleted(routine.id))}
                      onEdit={handleEdit}
                ...
                {todayMood && (
                  <div className="mt-6 p-4 rounded-lg bg-muted/50">
                    <p className="text-sm text-muted-foreground mb-2">
                      Última atualização:{' '}
                      {todayMood.updatedAt && format(new Date(todayMood.updatedAt), "HH:mm", { locale: ptBR })}
                    </p>
                    {todayMood.emotionIds && todayMood.emotionIds.length > 0 && (
